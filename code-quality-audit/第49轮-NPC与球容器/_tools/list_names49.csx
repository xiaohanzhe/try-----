...
// 第49轮 · 导出全部 code / script / object / sprite 名字（一行一个），便于本地检索
// 产出 <data.win 同级>/names49.txt
// （与第48轮 list_names48.csx 同构，仅改文件名，避免覆盖上一轮证据）
EnsureDataLoaded();

string dir = Path.GetDirectoryName(FilePath);
string outTxt = Path.Combine(dir, "names49.txt");
