...
【编码说明】本文件由 Python 以 UTF-8（不带 BOM）写入。
早期几轮的 `_evidence/*.txt` 是用 PowerShell 的 `*>` / `Out-File` 捕获原生程序 stdout 生成的，
那会把输出字节按 GBK 解码、再按 UTF-8 重写 → 中文被二次编码成谁也读不懂的方块字。
校验工具：`code-quality-audit/tools/check_evidence_encoding.py`（本文件里故意不贴乱码样本，
免得绊线判据把它自己认成乱码）。
