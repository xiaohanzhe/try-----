# -*- coding: utf-8 -*-
"""第 35 轮 —— 真机内部状态full-10min 探针（回答「为什么 89 秒零移动」）。
...
做法：
    ...
    写进 jsonl，跑 10 分钟。这份数据既是"用户视角监测"的第二通道，也是
    定位静默问题的证据。
"""
import os
import sys
...
OUT = os.path.join(EV, "10min_内部状态探针.jsonl")
ERR = os.path.join(EV, "10min_内部状态探针_错误.txt")
