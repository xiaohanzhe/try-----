...
# C10d —— **有意**不做的一步：流式层**不**删句中「括号动作」（2026-09-19）
# 为什么不做：句中删掉一段文本天然不前缀安全（`（轻轻敲` 已显示、`)` 到齐那一段才消失）；
# 而若改写成"删除未闭合括号之后的内容"来保前缀安全，会误伤 Ralsei 用括号讲心里话的
# 正常写法（原作 26/853 条以括号开头）。所以该删除**只留在收尾层**
# （`main._clean_ai_reply` 的 0b 步），代价是极少数回复(探针实测 18/574)的括号动作
# 在收尾那一刻被摘掉、屏幕上闪一下。
# ⚠️ 这条是**有意锁住的分工**：将来若要把删除挪进流式层，必须同时更新 C10 的边界清单。
ok('C10d 流式层不删括号动作（该删除有意只留在收尾层，见 C10d 上方注释）',
   san('嗯…（轻轻敲了下键盘）好的') == '嗯…（轻轻敲了下键盘）好的',
   repr(san('嗯…（轻轻敲了下键盘）好的')))
