...
# —— A22e/A22f：第三形态「描述性旁白」闸（2026-09-22 第27轮加）——
# 洞在哪（A22/A22c 都没覆盖）：A22 只抓**动作词开头**，A22c 只保**心里话**，
#   于是"从外部描述自己状态"的括号两边都不管 —— 真实对话探针实测漏网样本：
#     （心里有点慌，怕自己做得不好）／（我说完有点愣了一下）
#     （心里有点闷，像没打开的窗）／（语气轻快点，但没太夸张）／（心里一紧）
#   取证 `音27侦察/_evidence/probe_paren_census.txt`（漏网 1/1）与
#        `probe_ab_fragment.txt`（真实回复里出现 4 种该形态）。
#   用户口径（本轮）：「不想在和她聊天的时候出现那种失真的样子」——
#   括号旁白是"扮演痕迹"最直接的暴露（他写着"我是用打字的"却在括号里演动作）。
#
# ⚠️ 本轮**先做错了一版**（值得留下教训）：第一版用"开头必须是 其实/……"的
#   窄白名单，结果把合法心里话一起误杀 —— A22c/C19 实测报红
#   （`（我想想该怎么说）`/`（抱歉，我不是故意的）`/`（笑不出来）`/
#   `（因为……我一直都是一个人）` 全被清掉）。
#   正确口径是**抓"描述性叙述"的形态特征**（`_NARRATION_INNER`），不抓首词白名单：
#   叙述 = 以"心里/语气/沉默/愣了…"这类**从外部描述自己**的词开头；
#   心里话 = 第一人称的话语（其实…/因为…/抱歉…/我想想…）→ 必须原样保留。
_PAREN_NARRATION = [   # (输入, 期望输出) —— 期望 = 旁白被清掉后剩下的东西
    ('诶？那个……我可能说得不太好。（停顿了一下）', '诶？那个……我可能说得不太好。'),
    ('（心里有点慌，怕自己做得不好）嗯……', '嗯……'),
    ('（心里有点闷，像没打开的窗）', ''),
    ('（语气轻快点，但没太夸张）', ''),
    ('（心里一紧）', ''),
    ('（我说完有点愣了一下）', ''),
    # 验收实测又出现的三个「心里…」变体（2026-09-22 真机跑出来的）：
    #   `（心里有点小激动）`/`（心里有点发烫）`/`（心里有点乱）`
    # 收进来是因为**它们不是手写的假想样本，是产品真吐过的**——
    # 锁里放真样本，才能在下一轮改动时真的兜住。
    ('（心里有点小激动）', ''),
    ('（心里有点发烫）', ''),
    ('（心里有点乱）', ''),
]
ok('A22e 描述性旁白（心里…/语气…/愣了一下…）被清掉，剩下的台词放行',
   all(E.strip_action_parentheticals(a) == b for a, b in _PAREN_NARRATION),
   [(a, E.strip_action_parentheticals(a)) for a, b in _PAREN_NARRATION
    if E.strip_action_parentheticals(a) != b])
# 反向控制：**同一批心里话必须仍然一条都不动**（证明新闸没有扩大误杀面）。
# 与 A22c 共用样本集，但这里是在"新闸已上线"之后再断言一次 ——
# A22c 证"老闸不误杀"，A22f 证"新闸也没把误杀面做大"。两条都要，缺一不可。
# ⚠️ 判据边界（验收时亲自踩过）：`（我好像也不太懂你说的是什么……）` 是**第一人称的话**，
#   属于"括号讲心里话"，**必须放行** —— 它与"描述性旁白"的分界是
#   "这段到底是不是角色的**话语**"，不是"带不带我/心里"。
#   别把这条也当成漏网去补，那正是第27轮第一版误杀心里话的同型错误。
_INNER_KEEP_MORE = list(_INNER_PAREN) + ['（我好像也不太懂你说的是什么……）']
ok('A22f 反向控制：新闸上线后，括号讲心里话仍然一条都不动',
   all(E.strip_action_parentheticals(x) == x for x in _INNER_KEEP_MORE),
   [x for x in _INNER_KEEP_MORE if E.strip_action_parentheticals(x) != x])
