...
# —— A12d：语气示例区**不得再出现可被逐字搬走的固定短句**（2026-09-22 第27轮加）——
# 为什么单列一条：本轮 A/B 实测（`音27侦察/_evidence/probe_ab_fragment.txt`）
#   旧「碎片」区是**按场合分组 + 冒号 + 固定例句**的格式：
#     「坏消息的时候用：是这样啊……／那……现在还好吗／我听着呢，你慢慢说／……嗯。」
#   4B 把它理解成"这个场合的标准起手式"，逐字搬到回复开头 ——
#   同一模型、同一温度下，开头复读率 **61%**；把固定例句整节删掉后降到 **28%**。
#   用户口径（本轮）：「不想在和她聊天的时候出现那种失真的样子」——
#   这种"每条回复都从同一句起跳"的模板感，正是失真最刺眼的形态。
# 判据：示例区里不许再出现"按场合分组 + 冒号直接跟例句"的句式。
#   `坏消息的时候用：` 是旧格式的**原型串**，它一旦回归就说明有人在往回改；
#   `（起音含混）` 之类括号注释是新格式的正常写法，不算例句 —— 故只锚旧格式。
# 反向控制：`然后才是声音` 必须存在（证明示例区还在，否则本断言退化成恒真）。
_A12D_OLD_FORM = ('坏消息的时候用：', '好消息、好奇的时候用：', '一急、一慌：',
                  '不知道怎么接话：', '被夸、害羞：', '不自信、自嘲：')
_A12D_HITS = [s for s in _A12D_OLD_FORM if s in PERSONA]
ok('A12d 语气示例区不再含"按场合绑定的固定例句"（4B 会把它们当起手式逐字搬）',
   not _A12D_HITS and '然后才是声音' in PERSONA,
   '旧格式残留=%r 新区锚=%s' % (_A12D_HITS, '然后才是声音' in PERSONA))
# A12e：反"编造共同经历"规则必须存在（第27轮加，第二个失真来源）。
# 实测样本：「我们那时候总坐在石阶上，谁也不说话」—— 用户从没参与过黑暗世界，
# 模型把"我自己的过去"讲成了"我们俩的共同回忆" = 替他伪造记忆。
ok('A12e 明令"不许把那边的往事编成俩人共同经历"',
   ('不能编成' in PERSONA or '就是在瞎编' in PERSONA)
   and '没去过黑暗世界' in PERSONA.replace('他都不在场', '没去过黑暗世界'),
   '反编造规则缺失')
...
# A12c —— 第三条规则里的**篇幅约束必须排在声音示例之前**（第二十二轮）。
# 用户口径（本轮）：「那就加」—— 就"聊到设定时回答偏长"加一条长度约束。
# ⚠️ 为什么单列一条断言，而且断的是**位置**不是"字符串在不在"：
#   这条约束是"碰到事该怎么做"（规则），示例是"声音长什么样"。
#   persona 自己在收尾处写死了喂法：「先记三条…这三条比下面那些声音重要得多」。
#   若把约束塞到示例那一区，它就从"规则"降格成"示例"，
#   而**示例区全是短句** —— 模型学短句的样子，长度约束反而被稀释掉。
#   所以位置是本条断言的全部内容：只断"在不在"会被"随便塞哪儿"糊弄过去。
#
# 2026-09-22（第27轮）：断言里的"示例区起点"锚点必须同步换名 ——
#   本轮把「碎片」整节重写成「声音」，因为原「碎片」是**按场合分组的固定短句清单**
#   （「坏消息的时候用：是这样啊……／…」），4B 把它当成"这个场合的标准起手式"
#   逐字搬到开头（A/B 实测：开头复读率 61% → 28%）。改版后不再有任何可逐字搬的固定句，
#   所以锚字符串从 `坏消息的时候用：` 换成新区的起点 `然后才是声音`。
#   ⚠️ **断的是"位置关系"，不是那个字符串本身** —— 换名不降低鉴别力：
#   只要篇幅约束被挪到示例区之后，本条仍会红。
_A12C_RULE = '都别一口气倒完'
_A12C_FRAG = '然后才是声音'
ok('A12c persona 有"别一口气倒完"的篇幅约束，且在声音示例之前（规则区）',
   _A12C_RULE in PERSONA
   and 0 <= PERSONA.find(_A12C_RULE) < PERSONA.find(_A12C_FRAG),
   'rule@%d voice@%d' % (PERSONA.find(_A12C_RULE), PERSONA.find(_A12C_FRAG)))
