# -*- coding: utf-8 -*-
"""第34轮提交 + 推送 + 三方核验（win-git-utf8-push 铁律 0 / 0.1 / 0.2 / 0.3）"""
...
MSG  = r"E:\Download\_tmp\_commit_msg_34.md"
OUT  = r"E:\Download\_tmp\_push_result_34.txt"
...
def run(args, timeout=90, **kw):
    """统一带硬超时；返回 (rc, stdout_str, stderr_str)。-999 = 超时。"""
    try:
...
try:
    # [0] 提交信息首 3 字节自证（不能是 BOM 239,187,191）
    with open(MSG, "rb") as f:
        head3 = list(f.read()[:3])
    log("[0] msg first3=%s (must NOT be [239,187,191])" % head3)
    ...
    log("[2] commit rc=%d out=%s err=%s" % (rc, o.strip()[:400], e.strip()[:400]))
    ...
    log("[4] status=%r" % o.strip()[:1500])

    # [5] 取凭据（input 必须 bytes！）
    cred = None
    try:
        p = subprocess.run("git credential fill", cwd=REPO, shell=True,
                           capture_output=True, timeout=60,
                           input=b"url=" + URL.encode() + b"\n\n")
    ...
    if not pw:
        log("!! 无凭据 → 跳过 push（commit 已落本地，不会丢）")
        flush()
        sys.exit(2)

    b64 = base64.b64encode(("%s:%s" % (usr, pw)).encode("ascii")).decode("ascii")

    # [5b] 候选代理端口（env 优先 → 7897 → netstat LISTENING），socket 快筛
    import shutil
    cands = []
    ...
    for port in cands:
        try:
            s = socket.create_connection(("127.0.0.1", int(port)), timeout=1.5); s.close()
            alive.append(port)
        except Exception:
            pass
    log("[5b] cands=%s alive=%s" % (cands, alive))
    if not alive:
        log("!! 无可用代理端口 → 跳过 push")
        flush()
        sys.exit(3)
    ...
    for port in alive:
        ...
        common = ["-c", "credential.helper=",
                  "-c", "http.proxy=" + px, "-c", "https.proxy=" + px,
                  "-c", "http.extraheader=Authorization: Basic " + b64]
        for i in range(1, 4):
            rc, o, e = run(["git"] + common + ["push", "origin", "main"], timeout=90)
            log("[6] push port=%s #%d rc=%d out=%s err=%s"
                % (port, i, rc, o.strip()[:200], e.strip()[:300]))
            if rc == 0:
                pushed = True
                break
        if pushed:
    ...
    # [7] 核验：ls-remote 独立构造 argv（绝不切片复用）+ rev-parse 交叉
    match_remote = "NO"
    remote_line = ""
    for port in alive:
        px = "http://127.0.0.1:" + port
        ...
        for i in range(1, 6):
            ...
            ln = o.strip()
            log("[7] ls-remote port=%s #%d rc=%d out=%s err=%s" % (port, i, rc, ln[:200], e.strip()[:200]))
            if ln:
                remote_line = ln
                if local and local in ln:
                    match_remote = "YES"
                break
        if match_remote == "YES":
            break
    log("[8] ls-remote=%s" % remote_line)
    log("[8b] MATCH(ls-remote)=%s" % match_remote)

    rc, o, e = run(["git", "rev-parse", "origin/main"])
    log("[9] rev-parse origin/main=%s (rc=%d)" % (o.strip(), rc))
    log("[9b] MATCH(rev-parse)=%s" % ("YES" if o.strip() == local else "NO"))

    rc, o, e = run(["git", "status", "--porcelain"])
    log("[10] final status=%r" % o.strip()[:1500])
