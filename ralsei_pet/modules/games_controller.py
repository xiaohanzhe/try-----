...
class GamesController(object):
    ...
    # ------------------------------------------------------------------
    # 游戏输入分派（W1-7 搬运区，**方法体逐字来自 main.py L5973-6009**，
    # 唯一改动：无。本方法不打日志、不新增 import、不改写任何一行）
    #
    # 为什么它最后一个搬进来
    # ----------------------
    # W1-3 当时**刻意**把它留在宿主，理由是它经
    # `self._abort_hide_and_seek(reason='user_quit')` 依赖 W1-2（躲猫猫），
    # 搬走会引入跨控制器依赖、违反 Wave 1 的「零互写」前提。
    # W1-2 落地后 `_abort_hide_and_seek` 进了 `HideAndSeekController`，
    # 该依赖改由本类的 `__getattr__` **第 3 条白名单**（扫描宿主
    # `_CONTROLLER_ATTRS` 里的**兄弟控制器类**）承接 —— 前提成立，故搬入。
    #
    # 调用方**一个字符都不用改**：`dialogue_ui.py` L1249 写的是
    # `self.parent.handle_game_input(user_input)`，`parent` 是宿主实例，
    # 宿主 `__getattr__` 会把找不到的名字转给本控制器。
    # ------------------------------------------------------------------
