...
def strip_action_parentheticals(text):
    """删掉台词里夹带的「括号动作旁白」，返回剩下的台词。

    只删**括号内以动作/发声动词开头**的那一段（判据见 `_ACTION_PAREN_RE` 上方的注释），
    所以 Ralsei 用括号讲心里话的正常写法（「（……其实我有点怕）」）**不受影响**。

    返回空串的两种情况（语义都是"这条不能作为台词用"，由调用方按判退处理）：
      * 整句只有一个括号动作（删完什么都不剩）
      * 删完只剩标点/空白（没有任何实义字符）

    注意：这里**不做** `looks_like_narration` 那类"整句作废"的判断 ——
    句首括号的整句叙述交给那道老闸，两道闸分工不同（见上方注释）。
    """
    if not isinstance(text, str):
        return ""
    if not text:
