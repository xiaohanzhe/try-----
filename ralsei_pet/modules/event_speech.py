...
EVENT_TIERS = {
    ...
    "game_thanks": TIER_AI,
    # —— 第六批（第52轮）：睡眠/唤醒的拟声与反应 ——
    # 原本是写死的罐头（`zzz... 晚安，做个好梦！` / `zzz... 我困了...` /
    # `唔...别吵...` / `嗯？什么事？`），每次入睡/被吵醒都是同一组。
    # 用户口径：「还有把他内置的对话去掉！！！！」「聊天系统全权由7B接管」。
    # ⇒ 一律 **pool=None**（AI 不可用就安静地睡/醒，不甩写死的 zzz）。
    "sleep_enter": TIER_AI,     # 进入睡眠（含 23:00 就寝）
