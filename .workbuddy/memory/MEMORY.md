...
- ⭐⭐ **代理端口必须"逐端口实测 CONNECT"再选**：第 34 轮续二实测 **`7897` 通**（CONNECT→200→TLS 0.04s）、
  **`7375` 是死代理**（CONNECT 全 timeout）。**按端口升序盲试 = 每端口 25s×5 轮 ⇒ 单次 push 卡 19 分钟**，
  且被迫中断**不会留下输出文件**，看起来像"网络故障"。
  ⇒ **先纯 socket 层探测**（`CONNECT host:443` 看响应首行），**再**跑 git；`credential fill` 也要加 `timeout`。
  ❗`github.com`/`api.github.com` **DNS 解析成 `127.0.0.1` 是代理 TUN 的正常接管**（裸连 TLS 握手成功、
  证书 `CN=github.com` 正确），**不是劫持、不是故障** —— 别据此下"网络不通"结论。
...
- **探针不保真 = 报假问题**（**已踩 4 次**）→「**能从源码拿的别 import；能 import 的别重写；不得不重写必须锁等价**」；
  ❗❗**"分类器太窄"和"闸有洞"长得一样** → **必须拿产品函数逐条复核，不能只看探针计数**。
  ★★ **任何"缺陷成立/修复无效"的结论，先复核探针**（§23.12.1 抚摸判据**连续三次失真**
  ⇒ 最终**缺陷不成立、修复已回退**；§23.14 本轮**三条假设全被推翻**，依据见详版）。
...
- ★ **能从运行时读到的量就别推算**（F34-3：`sprite_size=100` 反推会得"窗口=100×100"的**反向结论**，
  真机读一次得 **138×94**）。**零可见后果的"口径不一致"归 P3、别顺手改**；
...
## 5. 人味改造线 ★ 动手前**必须 Read 详版 §23.13.1**（细则全文都在那；速查本只留否决线）
...
- **人设单一真源 = `assets/ralsei_persona.md`**（每次读出来当 `system`）。**别只写 Modelfile**
  ...
  persona **是 prompt 不是文档**（禁 markdown）；**❗不许叫"主人"**（persona + 代码两层）；
  **上限 K3 < 10000 B**（现 **9946 B**）⇒ **加内容前必须先瘦身**。
- ★★ **失真头号来源 = persona 里"可逐字搬走的固定例句"** ⇒ **示例区不许"按场合分组 + 冒号 + 固定例句"**
  （该**格式本身**就在教照搬）。锁 **A12d**。
  ★★ **"口癖多"≠ 全砍**：**真口癖**（真结巴仅 **8.3%**）**属人物真实性、别砍**；**固定模板句必须砍**。
- **护栏 `_clean_ai_reply(reply, recent=)` 顺序 = 优先级**：0a 句中括号动作 → 0b 剥 markdown → 0c 禁说清单
  → 1 自问自答截断 → 2 车轱辘话判退（**只看 recent**）→ 3 超长截断；❗**0a 先于 0b**；
  **判退后必须重采样一次**、**判退不 append 进 recent**。
  ★ **括号旁白判据 =「抓形态特征」不是「首词白名单」**（窄白名单**误杀合法心里话**）；**误杀比漏过更糟**。
  ❗**刻意不加裸 `笑`**（会误杀 `（笑不出来）`）。
- **❗`main.AI_REPLY_MAX_CHARS` 现 220**；**换底座/调 num_predict 必重跑 `measure_token_ratio.py`**。
- `relationship.py`（`TRUST_INITIAL=0.12`，`harsh` 别变死事件）/ `event_speech.py`（档位白名单、唯一出口
  `speak_event`、`pool=None`→沉默）、`ralsei:v4` 回退法、`strip_action_parentheticals` 的三形态判据、
  **S8 流式 `iter_lines(chunk_size=1)`**、**A12e 反伪造共同经历** → **细则全在详版 §23.13.1**。
...
## 7. 场景系统线 / 原作机制 ★ 动手前**必须 Read 详版 §23.13.2**（细则全文在那）
用户口径：**「把原作的世界搬到桌面上…桌面也会被我当成一个场景」** + **「一切根据原作」**。
- ★★ **P0 判据 =「不切场景时零行为变化」**：`switch()` **只写状态**、不动画面/定时器/物理
  （**一旦 P0 就改画面，这条最强判据就丢了**）。
- ★★ **路由是数据，不是代码**：`modules/scene_routing.py`（**纯标准库、零 Qt**）+ `assets/scenes/_routes.json`。
  **三律**：① 匹配不到返回 `None`（**绝不伪造默认路线**）② 坏规则跳过 ③ 未知键放行。
  ★ **只有 `follow_route` 允许调 `switch`**。
- ★ **原作机制**（GML 实证）：房间 ID **扁平整数** + **同一 id 不同章是不同房间** ⇒ **id 必须与 chapter 联合**；
  **Ch1~Ch5 = 87 间**。⚠️ 已解析的 `ROOM/SPRT/OBJT.json` **不可信**（指针基址错）。
- ★ **为什么现在不登记原作场景**：**素材还没做** → 切过去 = **空场景** = 「**世界突然变白**」，**比不切更糟**。
- `when_keywords` 的 OR 语义 / 排序规则 / `_as_set()` 坑 / 落地三步 / 游戏源文件路径 → **详版 §23.13.2**。
