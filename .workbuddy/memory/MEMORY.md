...
8. **状态：诊断 + L1 施工 + S8 流式 + S7 第一批事件台词（含 lean 修复）都已完成**
   ...
   **S7 只做了第一批 20 处**（`main.py` 131 → **112** 处 `add_dialogue`，全项目 191 → **172**，
   2026-09-18 第七轮重跑 `scan_event_lines.py` 核实），第二批**未做**；
...
- **lean 形状实测**（稳定 system + 变 user）：3B 首字 **351ms**、整句 **1750ms**、14.93 tok/s；
  7B 首字 **536ms**、整句 **4579ms**、7.35 tok/s。**冷 prefill：3B 93 / 7B 36 tok/s。**
- **⚠️ 更正上一轮的推断**：7B 首字 536ms **过得了** S7 的 1200ms 门槛 →
  **否掉 7B 的不是首字，是"整句慢 2.6× + 质量更差"**。上一轮那个判断是从**冷前缀**外推的，
  而 lean 是缓存命中路径 —— **同一条链路的"冷/热"是两个量，不能互相外推**（第十一轮"假冷"同坑第 2 次）。
- **质量（8 条输入，两轮复现）**：3B 问题 2/8、平均 28.2 字；7B 问题 5/8、平均 44.2 字、5556ms。
  7B 的两条退化是**结构性**的：① 三个 persona 示范场景**全抄**（3B 只抄 q2，两轮都是）；
  ② "黑暗世界"话题**两轮都拉 Kris** 并写成长文（111~161 字 / 6~9 句 / 12~17 秒），
  而 persona 明写"不要把主人卷进剧情里"。
- **4B 候选 `qwen3:4b-instruct-2507-q4_K_M`（非思考，2.5GB）未能验证**：本机到 Ollama CDN
  连接会"传一阵就被掐断"，三次尝试停在 **95.4%**，残片保留（续传只剩 116MB）。工具
  `pull_model.py` / `pull_with_retry.py`。**不挡路** —— 下面两条主因与底座无关。
- **⚠️ `ollama pull` 在客户端设 `HTTP_PROXY` 对 daemon 无效**（pull 只是客户端，真正下载的是 daemon，
  它用**自己启动时**的环境），表现为 `TLS handshake timeout` —— **别据此以为网络不通**。
  要改 daemon 代理必须重启 daemon。

### **真瓶颈：不在底座（第七轮最重要的结论）**
1. **persona 的「主人：/我：」示范是逐字进模型的提示词 → 会被照抄，且底座越强抄得越狠**
   （7B 3/3 场景、3B 1/3）。建议改成**"改写目标"句式**（描述而非成品台词），或把
   "今天上班好累啊"（最高频抱怨场景）换成低频场景。⚠️ 会牵动 `persona_chat` 的 **C7/C8 锁**。
2. **对话链路没有任何"禁说清单 / 出戏词"的代码级拦截（已核实的洞）**：
   `guard_reaction`/`looks_out_of_character` **只挂在事件链路**（`main.py:5368`，`speak_event`）；
   对话链路只有 `_clean_ai_reply`（`main.py:7746`），它只做四件事 ——
   **剥 markdown / 截自问自答 / 判重播 / 超长截断，一个字都不查禁说清单**。
   persona 第 40–41 行那句"这些话我不说"**只是提示词，无代码兜底**。实测已漏出变体：
   「**有什么我可以帮忙的吗**」（persona 写的是"有什么可以帮你的吗"）、
   「我**能感受到**你的疲惫了」（persona 写的是"我理解你的感受"）→
   判据要覆盖**变体**，别只写标准句。**修的时候注意**：判退→重采样→仍可能落到内置规则台词
   （已知陷阱），不是简单加个 `return None` 就完事。
3. **顺带（纯文案、零成本）**：`react_to_desktop_element`（`main.py:3981`）写死的
