...
> **全文（§0–§18）在 `参考-契约与历轮（详版）.md`**（**不自动注入，改动前 Read**）。
...
## 1. 环境（详版 §1 / §18）
- ⚠️ **Bash 工具常整个坏掉**（`dirname`/`ls`/`head`/`cat` 全 127）→ **一律 Python + PowerShell 工具**；
  `Glob`/`Grep`/`Read` 照常。**路径经 Bash 传 Python 用正斜杠**；❗`2>$null` 报 `ambiguous redirect`；
  ❗**别从 Bash 里调 PowerShell**；PS **stdout 常不回传** → 命令内 `Out-File -Encoding utf8` 再 Read。
- **起进程**只能 `&` + `run_in_background:true`；**杀进程**用 Python+psutil；**删文件**用 `[System.IO.File]::Delete()`；
  **`wmic` 已移除** → 用 `Get-CimInstance`。
- Python 一律 **`C:\Python311\python.exe`**（PyQt5/pywin32/bs4/psutil/jieba）—— G2 与所有套件必须用它。
- **代理端口现查为准**；**单端口一次成败不是结论**。**git 用 `shutil.which("git")`**。
- **E 盘会掉线**；❗**E 盘 exFAT 不能当探针沙箱** → 用本地 NTFS `%TEMP%\`。
...
- **`github.com` 主站不可达而 `api.github.com` 仍 200** 时会 push 失败 → **提交照做 + 如实报告 + 稍后重试**，
  **别怀疑凭据或改 git 配置**。
- ⭐ **仓库两套换行口径**（`autocrlf=true` + 无 `.gitattributes`，详版 §8.6.5）：双口径 14 个 / 单口径 58 个
  （含 `main.py`、所有 `*_controller.py`）⇒ **编辑必须保持原 EOL**；逐字节参照系只能用 `git cat-file blob`。
...
- ⭐**能上 AST 就上 AST**：`code_only_src()` **会剥 STRING token** → 断言**字面量**必恒假/恒红（**踩 7 次**），
  它只适合断言**标识符/运算符**；含字面量走 `code_no_comment()`。**零引用筛查一律走 AST，不走正则**。
...
- **A/B 纪律**：① 两侧都真跑到 ② **先断言 A ≠ B**（"旧值"若已提交 → A==B；取**父版本** + md5 相同即 `SystemExit`）
  ③ 桩打在「方法体里 `self` 的那张类」上 ④ **失效谱第 4 形态「参照系不是父版本」最险** ⑤ 断言分层
  ⑥ **A/B 只证"两版等价"**。**对照组重复跑、先量尺子公差**（±2 条＝噪声）；**计数会被反向激励** → **先读原文再看计数**。
...
## 8. 场景系统线（第 28–29 轮，**全文详版 §16/§17/§18**）
...
- **第 29 轮 路由层**（`52d5514`）：`modules/scene_routing.py`（**纯标准库、零 Qt**）+ `assets/scenes/_routes.json`；
  锁 `scene_routing`（**106 项**）。G2 **1358/26**。
...
## 9. 历轮索引（细节去详版 + `.workbuddy/memory/<日期>.md` + 对应报告）
...
## 10. 🔴 待用户裁定（**开工前必看**）
1. **7B 怎么处理**（用户要求「接7B试试，等待时间 1~3s 内可接受」）：
   ★★★ **前提已纠正**：用户说「显存32GB」——实测是 **Intel Core Ultra 5 125H + Intel Arc 核显 + 31.6 GiB 内存**，
   **无独显、无独立 VRAM**；**Ollama 跑在纯 CPU 上**（`library=cpu`、`size_vram=0`、**无 vulkan/cuda 后端 dll**）。
   ★★ **4B 实测**（**219 次真实推理**，源 `server.log`；证据 `_evidence/4b_server_log_perf.txt`）：
   **avg 8.07 tok/s**；**12 tok=1.49s / 20 tok=2.48s / 40 tok=4.96s**。
   ★★ **真瓶颈不是生成速度而是 prompt eval**：prompt 涨到 2000+ token（人设+记忆+世界观）时
   **prompt eval 要 35~47 秒**（短 prompt 仅 116~700 ms）→ **决定首字延迟的是 prompt 长度，不是模型大小**。
   7B 外推 ≈ **4.61 tok/s**（**全是外推值，非实测**；7B 仍在下载重试，manifest 未注册）。
   待裁定：**装好备用 / 不要 / 是否试 Vulkan**；**「1~3s」指首字延迟还是整句说完？**
