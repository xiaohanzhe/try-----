...
回归锁 `persona_chat`(156)／`s8_stream`(69)／`s7_event_speech`(138)，均进 G2、不联网。**人设单一真源 = `assets/ralsei_persona.md`**（**别只写 Modelfile** —— messages 的 system **整体替换**它）；**禁 markdown**、**❗不许叫"主人"**、**K3 < 10000 B**（现 9946 B）。★★ **失真头号来源 = "可逐字搬走的固定例句"**（锁 **A12d**）；**"口癖多"≠ 全砍**（真结巴仅 8.3%）。**护栏 `_clean_ai_reply(reply, recent=)` 顺序 = 优先级**（0a 括号动作 → 0b markdown → 0c 禁说清单 → 1 自问自答截断 → 2 判退（**只看 recent**）→ 3 超长）；**判退必重采样**、**不 append 进 recent**。**`AI_REPLY_MAX_CHARS` = 220**；换底座**必重跑 `measure_token_ratio.py`**。余下见 §23.13.1：`relationship.py`(`TRUST_INITIAL=0.12`)／`event_speech.py`(`speak_event`、`pool=None`→沉默)／`ralsei:v4`／`iter_lines(chunk_size=1)`／**A12e**。
...
## 7. 场景系统线 / 原作素材 ★ 动手前**必须 Read 详版 §16/§23.13.2/§36/§37/§39/§40/§41/§42**
用户口径：**「把原作的世界搬到桌面上…桌面也会被我当成一个场景」**＋**「一切根据原作」**＋**「把所有都拿出来啊，别就拿87个」**＋**「所有 room 排序和连接都按原作 = 场景复现，连动态效果也要做（喷泉这类）」**。
...
- ⚠️ **P1 前置**：`dump_rooms.csx` **缺 5 项**（层几何真名 `XOffset/YOffset/HSpeed/VSpeed`／Assets 层 `LegacyTiles` 几何／`TileData` 真矩阵／实例变换／连接边+帧表）⇒ **§42.6**；**瓦片数据本就在**（上轮打成 `"UInt32[]"` 是脚本 bug）。
...
- ★★ **记忆口径**：「**只要不影响读取就 OK，但影响的话就尽量别动**」＋41「**记忆不要随便修改，三思而后行**」⇒ 门槛：**实证超限或被要求** + 先补详版 + 逐令牌回验 + **改动守恒**，否则不动。★ **历轮口径**：36「**开始按照原版的路线**…**桌面当默认场景**」｜37「你自己反编译」｜38「**别一次性加载全部房间…走到哪亮到哪**」｜39/40 授权连续自主推进。

## 10. 历轮索引（**详版 §14–§41 有完整版** + 各轮报告）
**26–41** 复审+H4H5／AI 失真／场景 P0／路由／首字／7B／结巴／移动严查／素材反编译（**87 张背景**）／**1,013 场景+P0 接线（10/10）**／真背景 157（856 留空，**1547/33**）／40 **注入上限真相**／41 **原作复现取证**（§41/§42）。
`8f95f5e` `21b520d` `7b9d10a`（第38~39轮全表见详版 §41.10）

## 11. 🔴 待用户裁定（详版 §23.13.3 + §36.8 + §37.11 + §39.10 + §40.8）
1. ★★ **Q1 已拍板（41轮）**：「**所有 room 排序和连接都按原作 = 场景复现，连动态效果也要做（喷泉这类）**」⇒ 157 真背景只是起点，**856 留空项按原作补齐**；**Q4** 78/87 近似锚点随之一并升级。❗平铺画布口径（宽 1280／高 `1280×clamp(rh/rw,0.5,0.9)`）待重议。
...
3. 低优先：场景粒度｜BGM（`sound_manager` **无 BGM，只有 3 音效**）｜原作版权｜`dialogue_ui._rule_reply()` 罐头皮 ⇒「AI 关必沉默」是否也管"用户主动打字"｜`.gitattributes` 换行（**先议后动**）｜`src/` 顶层 `.bak`/`test_*`/`monitor_*.txt`（**等用户点头，不得批量删**）｜H4「文件反应」｜自启+预热留收尾。
...
5. ★★ **P1 前置**：扩 `dump_rooms.csx`（**5 项缺口见 §42.6**）+ 重跑五章 UTMT —— P1 与动态层的**共同硬前置**。
