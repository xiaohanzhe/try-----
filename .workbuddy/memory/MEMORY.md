...
- 改代码前先跑 G2 `code-quality-audit/regress/run_all.py`（现 **32 套件 / PASS=1505 / 全 IDENTICAL**）；它跑 `compileall` 会改写被跟踪的 `src/__pycache__/*.pyc` ⇒ 收工前 `git checkout --`；★ 改了断言/文案 → `--only <suite> --update`（**合并模式**），**别全量 update**。
...
- **38**：审查已写代码（21/21 + 36/36 + 11/11）+ 抓出「场景只有原作 8%」P0 → **全量落地 1,013 场景 / 61 区域分片**（G2 **1505/32**）+ **push 链路根因**（公开仓库陷阱 / helper-selector 弹窗 / GCM 直取 / TLS openssl）（§39，`6ea5998`/`f117aed`/`7e848f1`/`270e970`）
...
9. ★★ **38轮遗留**：**`load()` 仍未被产品调用**（`self.scene.<方法>()` 调用点 = 0）⇒ P0 接线最后一步；3 处代码瑕疵（`_FALLBACK_PRIORITY` 死常量 / `hasattr(self,'refresh_objects')` 恒假且与 `resolve_objects` 双名 / `main.py` 文档写"6 字段"实为 9）；`_original_rooms.json` 修（ch2 多 2 条 id 199/200、ch5 漏 5 条 id 205/222/224/225/230）。
