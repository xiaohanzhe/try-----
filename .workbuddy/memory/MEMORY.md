...
> **详细契约（§4.1–4.12）、验证脚本教训（§5）、人味改造线全文（§6.1–§6.10）、历轮索引（§7–§9）已拆到同目录
> `参考-契约与历轮（详版）.md`**（约 54KB，**不自动注入，用时 Read**）。本文件只留**每轮都必须遵守**的铁律、速查契约与当前状态。
...
- 报告放项目根 `代码质量复审报告_*.md`（人味线用 `人味*.md`）；证据进 `code-quality-audit/<轮次>/_evidence/`；
  侦察 `_recon/`（gitignore）。
- 改代码前先跑 G2 `code-quality-audit/regress/run_all.py`（**24 套件 / 1135 PASS / 全 IDENTICAL**）。
...
- 症状：退出码 128 且全静默 = 非交互 GCM 取不到凭据 → `git credential fill` 取密 + `-c credential.helper=`
  + `-c http.extraheader="Authorization: Basic <b64>"` + 代理；**端口现查**；重试 3–5 次。令牌别写进仓库文件。
- **❗核验外发用 `git ls-remote`，但"核验命令自己也会说谎"**：绝不从另一条命令的 argv 切片复用（踩过：
  拼出非法命令 → 恒空 → 误报"push 没落地"，而 push 早成功）。且 `ls-remote` **间歇性 `schannel` 失败**
  → 重试 3~5 次 + 用 `git rev-parse origin/main` 交叉验证。**先怀疑核验，再怀疑被测物。**
- **提交信息用 Write 写 UTF-8 文件 + `git commit -F <文件>`**（`-m @'...'@` 遇带空格英文引号会被 PS 5.1 拆 argv
  → 提交没发生、push 退 0 **假成功**）；**别用 `Out-File -Encoding utf8`**（加 BOM）。校验前 3 字节应
  `231,172,172`，`239,187,191` 即 BOM。**提交后必核 `git log --oneline -1`。**
...
- **❗核验命令自己也会说谎 —— 先怀疑核验，再怀疑被测物**：见 §2 第 2 条（`commit_r20.py` 切片复用
  拼出非法 `ls-remote` → 误报"push 没落地"，而 push 早成功）。**核验命令必须独立构造。**

## 6. 人味改造线（2026-09-18 ~ 19）（**全文见参考 §6.1–§6.10**）
报告 `人味*.md`；证据 `code-quality-audit/人味改造-2026-09-18/_evidence/`；
回归锁 `persona_chat`(**129**) / `s8_stream`(**69**) / `s7_event_speech`(**135**)，**均已进 G2**。**三套件都不联网、不调 Ollama。**
...
- **❗平级口径（第二十轮，用户硬要求）**：**不许叫"主人"**，用户与 Ralsei **平级**，他**不是为谁而来**
  ...
  `memory_system.recall_text` 的 `who` / `_role_marker_re()`（**保留"主人"**，识别旧遗留标记）。锁 **J1–J6 接线**。
- **❗世界观已从 persona 拆出**（第二十轮）→ `assets/ralsei_worldview.md`（11 块 `@@ key|label|triggers`）
  + `modules/worldview_recall.py` 按需召回（`MAX_BLOCKS=2` 硬上限；**无命中返回空串**；`lean=True` 跳过）。
  **动机是 KV 前缀缓存**（Ollama 只复用从头字节相同的前缀），persona **13140→9269B**。锁 K1–K10。
  **依据取原作语料不取我记忆**（`analyze_ralsei_worldview.py` 统计 853 条原台词）。
...
- **遗留**：① **真机实测（用户做，"还没办法实测"→ 整块搁置）**：遮挡 / 上下动手感 / 边缘掉落不生气而关窗生气 /
  **世界观召回 + 关系演进的真机行为探针** —— **契约级+单元级已锁，真机行为级未验**。
