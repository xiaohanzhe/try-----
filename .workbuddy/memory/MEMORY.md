...
> **全文（§0–§40）在 `参考-契约与历轮（详版）.md`**（**不自动注入，改动前 Read**）。
> 本文件只留**每轮必守的铁律 + 索引 + 待裁定**。
> ⚠️ 注入上限（**代码实证**，非估算）：**≤ 200 行 且 ≤ 25,000 字节**（按 `utf-8` **字节数**判，不是字符数；超限从尾部砍）⇒ 现 **118 行 / 24,216 字节 ✅ 在限内，但只剩 ~780 B 余量** ⇒ **下一轮加内容前必须先瘦身**（只做结构性合并 + 逐令牌回验）。
...
- 改代码前先跑 G2 `code-quality-audit/regress/run_all.py`（现 **33 套件 / PASS=1547 / 全 IDENTICAL**）；它跑 `compileall` 会改写被跟踪的 `src/__pycache__/*.pyc` ⇒ 收工前 `git checkout --`；★ 改了断言/文案 → `--only <suite> --update`（**合并模式**），**别全量 update**。❗**判据消息里别印绝对行号/会漂的计数**（第38轮：main.py 头插 19 行 ⇒ `round34b` 与内容无关地假 DIFF ⇒ 假 DIFF 会养成"看都不看就 update"的习惯）。
- ★ **只读优先、改动最小化**：审查阶段不改被审文件；修复一次只动必要处，每处配独立断言。
- ★★ **回归套件不许依赖"用后即删"的临时区**（第39轮）：临时文件一清，套件**不是报红、而是静默失去鉴别力**（读不到 ⇒ 全判 `none` ⇒ 断言全绿）⇒ **把判据要的事实蒸馏进仓库**（例：`第39轮/_evidence/原作房间背景溯源.json`）。
...
- ★ **命名口径 = 不译**：`区域中文名·尾段原样`（去掉命名空间与区域词）+ `name_raw` 溯源。**译名 = 伪造本地化**，违反「算不出 → 绝不伪装」。⚠️ **数字/单字母 token 是区分符，别当噪声丢**。926 个新场景里 **148 个已配真背景**（第39轮），余下 **仍 `bg: null`** —— **不编造区域代表素材**（近似档适用范围**零扩大**，锁 `bg_round39` E1/E2/E4）。
...
- ★★ **真背景只覆盖 15.5%（157 / 1,013 场景）**（第39轮落地 148 张，`bg/` 88→236）：1,251 房间里 **185 个有背景层 / 689 个有瓦片层** ⇒ 想更忠实只能"**按原作瓦片还原房间画面**"（856 个近似场景中 634 个所属房间有瓦片）。字段 `bg` + `bg_source`（真=`room.bg_layer`）+ `bg_asset`（原作精灵名，溯源）。**判据单一真源 = `第39轮/_tools/bg_common.py :: classify()`**。
- ⚠️ **P1 前置**：现有 `dump_rooms.csx` **没导出背景层几何**（`x/y/xscale/yscale/tiled/speed/层序`）⇒ 实测 6 个房间素材宽 < 房间宽一半（`dw_city_traffic_4` 480×480 / 房间 4600×480）＝**平铺重复绘制** ⇒ **P1 开工前必须扩 csx + 重跑五章 UTMT**。
- ★ **房间本身有尺寸分级**（320×240 小房间 / 640×480 标准 / 更宽滚动房间），**素材与房间基本 1:1** —— 别推断成"半分辨率靠 xscale"（第39轮真踩过，被 84/87 的数据推翻）。
...
- **38**：审查已写代码（21/21 + 36/36 + 11/11 + 15/15）+ 抓出「场景只有原作 8%」P0 → **全量落地 1,013 场景 / 61 区域分片** + **push 链路三根因** + **P0 接线（真机 10/10）** + 3 处代码瑕疵 + `_original_rooms.json` 修正（20/20/9/20/26）+ **修掉我自己的记忆上限错判据**（§39，`6ea5998`/`f117aed`/`7e848f1`/`270e970`/`af12a3b`/`21b520d`）
- **39**：**全量场景真背景落地**（157 真 / 856 近似留空 / 0 未配上；新增 148 张原作 PNG；接 `bg`+`bg_source`+`bg_asset`；修 87 处过期注释；diff +705/−383 精确等于设计值）+ **选层复核**（1/157 非最大层、2 处待复核）+ **判据侧 3 次失真 + 1 次我的错误推断**（"半分辨率"被数据推翻）+ 近似档零扩大加锁（G2 **1547/33**）（§40）

## 11. 🔴 待用户裁定（详版 §23.13.3 + §36.8 + §37.11 + §39.10；开工前必看）
1. **场景美术（39轮更新）**：**157 个场景已配真背景**（原作 PNG 原样提取）；❗**856 个仍留空** —— Q1：A 保持留空 / **B 按原作瓦片还原**（1,251 房间里 689 个有瓦片，最忠实、工作量大）/ C 沿用"区域代表素材"近似。78/87 锚点仍是近似（Q4）。❗平铺画布口径（宽 1280 / 高 `1280×clamp(rh/rw,0.5,0.9)`）待重议 —— 现已知道**房间本身有尺寸分级**。
...
9. ★★ **38轮遗留（P0 部分已结清）**：~~`load()` 未接线~~ / ~~3 处瑕疵~~ / ~~`_original_rooms.json`~~ **均已修**。**剩**：**路由重建**（现 26 条只覆盖 ch1~ch5 主线，1,014 场景可达面未铺开 —— 属设计决策，待用户过目）。
10. ★★ **39轮待裁定**：**Q1** 856 个场景的画面（留空 / **瓦片还原** / 近似）；**Q2** `ch1.kris_room` 的 bg 是 640×480 放大件（原作 320×240、其余 4 章同名场景都是原样）是否换回；**Q3** 2 处选层待复核（`torielclass` 取到**日语变体** `bg_lang_ja_torielclass` / `dw_castle_restaurant` 取到柜台层）是否定向覆盖；**Q4** 78 个近似锚点是否与 Q1 一起升级。★ **P1 前置（躲不开）**：扩 `dump_rooms.csx` 导出背景层几何 + 重跑五章 UTMT。
