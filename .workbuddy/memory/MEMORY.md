...
> **详细契约（§4）、验证教训（§5）、人味线全文（§6、§11）、网图（§10）、历轮索引（§7–§9）
...
- 改代码前先跑 G2 `code-quality-audit/regress/run_all.py`（**24 套件 / 1151 PASS / 全 IDENTICAL**）。
- 下载/生成物默认落 `E:\Download`（临时件 `_tmp\` 用后即删）；仓库内产物留项目目录；系统默认下载目录不动。
- G2 会跑 `compileall` → 可能改写被跟踪的 `src/__pycache__/*.pyc` → 收工前 `git checkout --`。

## 1. 环境（**全文见参考 §1**）
1. **优先用 bash + 把 PortableGit 的 usr/bin 加进 PATH**（补 coreutils，见第 4 条）；需走 PowerShell 时：
   **stdout 常不回传** → 命令内 `Out-File -Encoding utf8 <文件>` 再 Read（父目录不存在会静默 exit 1 → 先建目录）。
...
4. **代理端口现查为准**（`netstat -ano | findstr LISTENING` 或读注入 `HTTP_PROXY`）。❗**单端口一次成败不能当结论**：
   **每端口至少 2~3 次 + 跨端口重试（含直连）。** ❗**bash 缺 coreutils**（`grep`/`sed`/`cat`/`ls`/`base64` 全无），
   但 **PortableGit 自带一份**：`export PATH="/c/Users/23002/.workbuddy/binaries/PortableGit/versions/1.2.0/usr/bin:$PATH"`
   → 全部可用，**比"一律改 PowerShell"更省事**。（`github.com` 直连 443 不通、走代理则通；`api.github.com` 直连通 →
   **失败 ≠ 网络不通 ≠ 端口选错**，详见 skill `win-git-utf8-push`。）
5. **E 盘是外接盘、会掉线**（判在线看 `Get-Disk`）→ 本地中转站是可用性必需。
6. 清理守卫按目标路径**累计删除计数**（阈值 50）：症状 = 进程在 import 期就被杀。已在应用侧根治。
   **通则：见"计数恒定"先干掉触发源，别急着下确定性结论。**
7. **证据/报告一律让 Python 自己写 UTF-8**：绝不用 `& py x.py *> o.txt` / `| Out-File -Encoding utf8` 捕获原生
   ...
   → 去 BOM 必须 Python 重写。校验器 `code-quality-audit/tools/check_evidence_encoding.py`。
   **"能力自评失准"比"能力不足"更危险。**
...
## 2. git push（**完整配方 + 铁律 0（一个 Python 脚本跑完 add/commit/push/ls-remote）见 skill `win-git-utf8-push`**）
...
- bash 里可直接用：`git credential fill` 取 `username`/`password` → `base64 -w0` 拼 Basic 头 →
  `git -c credential.helper= -c http.proxy=http://127.0.0.1:<端口> -c http.extraheader="Authorization: Basic <b64>" push origin main`。
- **❗核验命令自己也会说谎**：`ls-remote` 绝不从别的命令 argv 切片复用，必须**独立构造** + 重试 3~5 次 +
  `git rev-parse origin/main` 交叉验证。**先怀疑核验，再怀疑被测物。**

## 3. 仓库与真机- 远端 `https://github.com/xiaohanzhe/try-----.git`（私有，未认证 401/404）；main→origin/main。
...
- **存储两层**：vault=`E:\RalseiMemory\`（读永远优先），staging=`%LOCALAPPDATA%\RalseiPet\`；`data_store` 是**唯一入口**；
  ...
  **jieba 是软依赖**，失败一律静默回落内置。**新增底层模块一律零项目内 import**（初始化环）。
- **世界观网图（二十一轮）**：块头下 `@@> 本键 -> 邻键, 邻键` 声明边；`load_graph()` → `(blocks, adj, dangling)`；
  `recall()` = 取种子 → **只在名额未满时沿出边补邻块，只扩一层、不越 MAX_BLOCKS**。
  **两铁律**：① **邻块不要求自己被 cue 命中**（加同门槛 = 扩图永不开火）；② **不做二阶扩图**。
  **添块必同时给反向边**。⚠️ **`A in rev[B]` 检反向是恒真判据**（正确判据 `b.key in adj[t]`）；
  **鉴别力体检必改文件**（`adj` 是解析期算的，改内存 → 假 P）。
- **建楼（本项目最贵的一条线，勿凭直觉重做）**：楼层 = 窗口的**可见区域**，`floor_visible_contains` 是**唯一判据**；
  **禁用 `Qt.WindowStaysOnTopHint`** → `_apply_pet_z_order()`（**必须在 `show()` 之后**）；坠落只看**层高比较**，
  起因按 `is_floor_valid(old_floor)` 分流；生气时长由 `_fall_reason` **纯派生**；**甩飞路径必显式 `self._fall_reason = None`**；
  跳/爬 `_jump_kind_for_span`；**不做“逐层小跳”**；⚠️ **`climb_1_*`=朝右 / `climb_0_degrees_*`=朝前 是用户当年原话口径，不许再改**。
...
- **回归锁必须有鉴别力**（两侧同值＝没测）；**正/负控制必须成对**；断言**断行为不断赋值**。
  ⚠️ **恒真判据比不写还危险**（二十一轮 `A in rev[B]` 恒真，掩盖图里 4 条真单向边）；**体检须"改文件"**（解析期值改内存 → 假 P）。
  ❗**鉴别力报红时先自问"夹具真的把破坏写进去了吗"**（二十二轮连报三次假红，全是夹具 bug：破坏作用在未改源 /
  锚点没删干净 / **三元表达式写反**）；函数体内引用后置全局 → `NameError` 被吞。
- **阈值型断言必须锚在"实测出的物理上限"上，不能是裸常数**（二十二轮：150 是 3B 时代拍的，换 4B 后
  **低于模型常态输出 → 闸门在砍正常回答**，且长期不可见）。**换底座/改 num_predict 必须重跑对应测量脚本复核。**
- **探针不保真 = 报假问题**（少复刻产品一步 → 报"（停顿了一下）泄漏"，而产品侧早剥掉了）→
  **报问题前先自查"夹具是不是少做了产品做的事"。**
...
- 离屏**无字体** → `QPainter.drawText` 静默不画；**删 Ollama 模型看 blobs 目录，删完必须真发一次 `/api/chat`**。

## 6. 人味改造线（2026-09-18~20）（**全文见参考 §6 + §11**）
报告 `人味*.md`；证据 `code-quality-audit/人味改造-2026-09-18/_evidence/`；回归锁
`persona_chat`(**145**) / `s8_stream`(**69**) / `s7_event_speech`(**135**)，**均已进 G2，都不联网、不调 Ollama。**
- **人设单一真源 = `assets/ralsei_persona.md`**（每次对话读出来当 `system`）。**不要**只写 Modelfile
  ...
  → **本机唯一可行的"训练"杠杆**；persona **是 prompt 不是文档**：不许 markdown。
- **persona 完整性锁**：A1–A11 + A12 三条接法规则 + A12b 反向控制 + **A12c 篇幅约束位置** + A23–A25 平级口径 + F1/F7。
...
- **❗世界观（二十一轮改网图）= `assets/ralsei_worldview.md` + `modules/worldview_recall.py`**：`MAX_BLOCKS=2` 硬上限；
  **无命中返回空串**；`lean=True` 跳过；**动机是 KV 前缀缓存**，persona **13140→9269B**。**依据取原作语料**（853 条）。
  锁 **K1–K20**；图契约见 §4；**端到端探针** `verify_persona_worldview.py`（`--no-recall` 为对照组）。
- **❗关系演进（二十轮）= `modules/relationship.py`**：四档**按用户原话顺序** distrust→guarded→warming→friend；
  `TRUST_INITIAL=0.12`（**从 distrust 起步**）。**信任度不可见**：只给**行为指令** + `_FORBID_LEAK` 禁泄；
  `describe()`（唯一带数字）只进日志。唯一写入口 `note(event)`；事件由**确定性** `classify()` 判定。
...
- **输出护栏 `_clean_ai_reply(reply, recent=)`（顺序 = 优先级，别乱调）**：0a 句中括号动作 → 0b 剥 markdown →
  ...
  （**只看"和自己最近说过的（recent）重复"**）→ 3 超长截断；**判退后必须重采样一次**。锁 C18b/C20–C23/H1–H4。
  **S8 流式**：`iter_lines(chunk_size=1)`（**流式收益的一半**）；流式只做"前缀安全"清洗，护栏负责"最终算数"。
- **❗篇幅上限（二十二轮）= `main.AI_REPLY_MAX_CHARS`，现 220**（原 150 是 3B 时代拍的，**在砍正常回答**）。
  由 `num_predict 256 × 1.359 字/token ≈ 348` 物理上限 × 63% 推出；锁 **M1–M5**（M2 = 落带 50%~75%，M5 = 防 C17 退化成恒真）。
