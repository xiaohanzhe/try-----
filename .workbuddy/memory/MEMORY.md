...
- 改代码前先跑 G2 `code-quality-audit/regress/run_all.py`（**24 套件 / 1161 PASS / 全 IDENTICAL**）。
...
1. **优先 bash + 把 PortableGit 的 `usr/bin` 加进 PATH**（补 coreutils）；`cd` 中文路径在 shim 里会
...
4. **代理端口现查为准**（`netstat -ano | findstr LISTENING` 或读注入 `HTTP_PROXY`）。❗**单端口一次成败不能当结论**：
   **每端口至少 2~3 次 + 跨端口重试（含直连）**；候选列表逐个试（注入值 → 7897 → 7890），push 成功即停。
...
11. ⚠️ **本沙箱的 Bash 工具经常整个坏掉**（`dirname`/`ls`/`mkdir`/`cd` 全 `command not found`，exit 127）
    → 此时**一律 PowerShell + Python**，`Glob`/`Grep` 工具照常可用。
...
- **6. 源级断言别用 `"字面量" in 源码`** → `code_only_src()`；含字面量的 needle 走 `code_no_comment()`（5 次踩）。
...
- **探针不保真 = 报假问题** → 口径「**能从源码拿的别 import；能 import 的别重写；不得不重写的必须锁等价**」。
...
- **❗世界观 = `assets/ralsei_worldview.md` + `modules/worldview_recall.py`**：`MAX_BLOCKS=2` 硬上限；
  **无命中返回空串**；`lean=True` 跳过（KV 前缀缓存）。锁 **K1–K20**。端到端探针 `verify_persona_worldview.py`。
- **❗关系演进 = `modules/relationship.py`**：四档**按用户原话顺序** distrust→guarded→warming→friend；
  ...
  唯一写入口 `note(event)`。**勿把下界抬到 TRUST_INITIAL**（会让 `harsh` 变死事件）。锁 L1–L16。
- **底座 = `ralsei:v3`（`qwen3:4b-instruct-2507-q4_K_M`）**：**采样参数必须落 Modelfile**
  （`/v1/chat/completions` **静默忽略 `num_ctx`/`repeat_penalty`**）；**⚠️ 换底座必须
  `ollama create <新tag> -f ralsei.modelfile`**（只改 config.json 无效）。
- **护栏 `_clean_ai_reply(reply, recent=)`（顺序 = 优先级，别乱调）**：0a 句中括号动作 → 0b 剥 markdown →
  ...
  ❗**0a 必须先于 0b**（否则 `*轻轻敲了敲键盘*` 的星号被 0b 吃掉 → 留裸叙述）。锁 C18b/C20–C23/H1–H4。
  **S8 流式**：`iter_lines(chunk_size=1)`（**流式收益的一半**）；流式只做"前缀安全"清洗。
- **❗篇幅上限 = `main.AI_REPLY_MAX_CHARS`，现 220**（原 150 是 3B 拍的，**在砍正常回答**）；由
  `num_predict 256 × 1.359 字/token ≈ 348` × 63% 推出。锁 **M1–M5**。
...
- **H4/H5 上帝类拆分**（用户排期"单独做"）：基线 `code-quality-audit/架构改造-H4H5/`（**勿重测**）；**S1/S2/S3 完成**；
  **G1 = 每项 PR 内做该专项零引用筛查**；**Wave 1 顺序 W1-3→W1-4→W1-1→W1-2→W1-6**（W1-3 ✅ / W1-4 ✅）；
  **旧行号已失效 → 开工前必须重跑 `scan_method_index.py`**。
  剩余：**W1-1（施法 2 方法/63 行）→ W1-2（躲猫猫 12 方法/323 行）→ W1-6（文件表 7 方法/399 行）**。
  - ✅ **W1-3 完成**（`d8e2878`）：`GamesController`（7 方法/258 行）搬出，`main.py` 10481→10265。
    口径「**只搬方法、不搬状态（转发壳 + 宿主 API）**」→ 状态**仍留在 `RalseiPet.__init__`**；
    方法体**逐字搬运**；控制器**不 import 任何项目内模块**。校验 `verify_w1_3_*` = 38/45/46 PASS + A/B 轨迹一致。
    **`handle_game_input` 刻意不搬**（调 W1-2 的 `_abort_hide_and_seek`）→ **待 W1-2 落地后随该项一起搬**。
  - ⚠️ **W1 转发机制铁律：双向 `__getattr__` 必须两侧都是显式白名单**，否则成环。
    ...
    **真机踩过**：`__init__` **L405** `init_movement()`→`randomize_movement_pattern()` 会 `getattr(self,'game_state',{})`，
    而 `game_state` 直到 **L426** 才赋值 → `RecursionError` **崩在构造期**。
    → **构造期崩溃 G2 抓不到**（只有 16 个套件会 `RalseiPet()`，部分还是桩）：自写 e2e（真 `RalseiPet()`）才抓到。
    **"方法体逐字等价" ≠ "产品还能跑"**，两者必须独立证明。
  - ✅ **W1-4 完成**：`VideoController`（**8 方法/289 行**）搬出，`main.py` **10278→9991**。
    刻意**不搬** `close_bilibili` / `enter_sleep_mode` / `check_entertainment_needs`（靠宿主 API 回落）。
    校验 `verify_w1_4_{unit,e2e,ab}` = **51/59/7 PASS，0 FAIL**；G2 **24/24 IDENTICAL 1161 PASS**。
    **全文（五条铁律 + 探针假差异 3 次 + 跨项待办）见参考 §8.2 / §5.2。**
  - ❗❗ **W1 搬运五条铁律**（W1-4 踩出，全属"逐字等价测不出来"）**：搬前必读参考 §8.2**。
    一句话速查：① `self` **当对象传出去**救不了 → `QTimer(self.p)` + 补 Qt 导入；
    ② 控制器**必须实现 `__setattr__`**（判据＝宿主**已拥有**该名；**必跳过 `'p'`**）；
    ③ **新状态名必须在宿主 `__init__` 预声明**；④ `_log` 用普通方法 `_log_()` 取宿主模块同一对象；
    ⑤ **锁的定位器跟着代码搬家**（`_find_func` 扫 main + controller），**禁止**弱化成"找不到就跳过"。
    **W1-3 `GamesController` 经 AST 不受铁律 2 影响**（状态走下标赋值）→ 待办：补同构 `__setattr__` 护栏。
