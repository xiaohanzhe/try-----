...
- ★★ **第27轮：「失真」的头号来源是 persona 里"可逐字搬走的固定例句"，不是模型不行**
  （真机 24 条实测开头复读 **62%**，且**只有 4 个起手式**覆盖全部命中）。
  **铁律：persona 的示例区里不许出现"按场合分组 + 冒号 + 固定例句"的句式** ——
  「坏消息的时候用：是这样啊……／…」这种格式**本身**就是在教 4B"这个场合该说这句"。
  A/B 实测：删掉全部固定例句 → 61% **降到 28%**；A 组剩的 28% 里那句
  「诶！真的吗？」**仍占大头（因为我还在 B 里留了一句当例子）** ⇒
  **只要那串字还在 persona 里，4B 就会搬；改成"情形描述 + 每次自己现造"才有效**。锁 **A12d**（禁旧格式回归）。
- **persona 体积上限**：`verify_persona_chat` 的 **K3 锁 < 10KB**（现 9752 B，已接近）→
  **下次往 persona 加内容前必须先瘦身**。
- **反"伪造共同经历"**（第27轮加）：用户**没去过黑暗世界**，模型会把"我自己的过去"
  写成"我们那时候一起…"（实测样本「我们那时候总坐在石阶上」）= **替他伪造记忆**。
  persona 已明令"只能说成我自己的过去，不能编成我们俩一起经历过"。锁 **A12e**。
...
- **★ 括号旁白共三形态，别只记一道闸**（第27轮补）：
  甲 **动作词开头**（`（歪头）`）→ `_ACTION_PAREN_RE`；
  乙 **句首括号**（`（我扶着墙站了起来…）`）→ `looks_like_narration` 整句作废；
  丙 **描述性旁白**（`（心里有点慌）`/`（语气轻快点）`/`（愣了一下）`）→
  `_NARRATION_INNER`（**第27轮加**；改前实测 5 种形态**全漏**）。
  ★★ **判据是"抓形态特征"，不是"首词白名单"** —— 第27轮第一版用窄白名单
  （只放过 `其实/……` 开头）**把合法心里话一起误杀**（C19/A22c 报红：
  `（我想想该怎么说）`/`（抱歉，我不是故意的）`/`（笑不出来）`/`（因为……我一直都是一个人）`）。
  **误杀 = 该说的话被吞掉，比漏过更糟。** 正确分界 = **这段是不是角色的"话语"**：
  第一人称的话 → 留；从外部描述自己 → 删。锁 **A22e（正控制）+ A22f（反控制，与 A22c 成对）**。
...
## 9. 第 27 轮 AI 失真治理（**全文见 `第二十七轮AI失真治理报告_2026-09-22.md`**）
- ✅ **已施工 + 真机验收 + 提交推送**（`685f98b` 产品 + `51919da` 记忆，`ls-remote` 核验 **SYNCED=True**）。
- **三源全治**：① 开头复读 **62% → 33%**（A/B 证成因果 61%→28%，正式版删光全部固定例句）；
  ② 括号旁白 5 形态全漏 → **全部被拦**（新增第 3 道闸 `_NARRATION_INNER`）；
  ③ 编造共同经历 1 处 → **0 处**（新增明令 + 锁 A12e）。
- **未达标但判为可接受**：33% 里剩 3 种全是**短促语气起音**（`诶！`/`诶、诶？`/`我、我……`）=
  **Ralsei 人设特征**（原作结巴是他的标志）→ 清零等于砍特征。**下一轮杠杆若还嫌模板感重 =
  采样侧（temperature/repeat_penalty），但本轮无证据支持动它**。锁 A12d/A12e/A22e/A22f（+4）。
- ★ **探针自报的误报教训**：验收脚本报"括号残留 4 处" → 拿**产品函数**逐条复核 = **3/4 是探针误报**
  （探针分类器只查 `其实/……` 开头 —— **犯的正是我第一版修复的同一个错**）。
  ⇒ **"分类器写得太窄"和"闸有洞"长得一模一样**，**必须拿产品函数逐条复核**。
