...
- 报告放项目根 `人味*.md` / `架构改造-H4H5/W1-*.md`；证据进 `code-quality-audit/<轮次>/_evidence/`
...
- **阈值型断言必须锚在"实测物理上限"**；**换底座/改 num_predict 必须重跑测量脚本**。
...
- **反控自己也会假**：① 场景要选"**必然命中**那一行"的；② 撤兜底要撤**对**名字（`_log` ≠ `_log_`）；
  ③ 必须加断言证明**好版本真的执行到了那行**。**"反控通过"比"探针假绿"更危险**。
  **防御性 `except` 会把 P0 变成静默**。
- **❗❗A/B 三大纪律 + 失效谱三形态**（全文见参考 §8.4）：① **必须守「两侧都真的跑到了」**（结果一致 ＋
  ...
  **别预设宿主类名**，stub **一次装齐整组方法**；③ **桩必须实现真契约**（缺 `Screen.center()` 会**两侧同抛**）；
  ...
  ⇒ **A/B 只能证"两版行为等价"，不能证"落点正确"**；凡涉及「**`self` 指谁**」的改动
  （铁律 1/3/4/6）**必须有非 A/B 的独立维度**（e2e 真实例）。
...
## 7. 历轮索引 / H4-H5 / 遗留（**详情见参考 §7–§9 + 今日日志**）
- **历轮 7–24**：**7–18** 联想三件套 / jieba / `data_store` 收口 / 建楼线 / DPI / 批次 A/B /
  S8 流式；**19** S7 batch2 + 底座 v3 + 人味四轮；**20** 世界观按需召回 + 平级口径 + 关系演进；
  **21** 世界观网图；**22** 篇幅 150→220；**23** 探针保真度六闸复刻 + 清存量；
  **24** H4/H5 **Wave 1 六项全做完** + 搬运铁律 8 条 + A/B 失效谱三形态。
- **H4/H5 上帝类拆分**：基线 `code-quality-audit/架构改造-H4H5/`（**勿重测**）；**S1/S2/S3 完成**；
  ...
  **旧行号已失效 → 开工前必须重跑 `scan_method_index.py`**；Wave 3 **不在范围**。
  - ✅ **六项均已推送**：**W1-3**（`d8e2878`）`GamesController` 7/258；**W1-4**（`2d288c9`/`cc03ca4`）
    `VideoController` 8/289；**W1-1**（`26bb8c8`/`9770e39`）`SpellFlowController` 4/290；**W1-2**（`e245f13`）
    `HideAndSeekController` 12/342，9713→9375；**W1-5** office 死代码清理；**W1-6** `FileSheetController`
    4 业务/469 行，9375→**9019**、方法数 175→**170**、删死代码 `create_person_name_table`；W1-6 三维
    UNIT 59/E2E 53/A/B 57 **全 0 FAIL**，G1 无新死代码，G2 **1164 PASS/0 FAIL/24 套件全 IDENTICAL**
    （`round5_smoke` 已合并重固）。**全文见参考 §8.1–§8.5。**
  - ❗**`scan_method_index.py` 只是线索，不是范围定义**（名字前缀启发式）——**四类偏差都出现过**：
    W1-1 `_tick_*`/`_cast_*` **漏标**；W1-4 `_*video*` **漏标**；W1-2 `_hide_ralsei` **误纳**；
    **W1-6 跨区散布**（7 方法横跨 L4222–4584 与 L8511–8591，相距 ~4000 行 ⇒「物理连续块」假设**不成立**）
    + **漏标第 8 方法 `_open_desktop_item_by_name`**（夹在 A 块中间、被 `handle_file_operation` 显式调用）⇒
    **照索引施工会留下"被搬走的方法所调用的方法" → 运行期 `AttributeError` 被 `except` 吞掉 = 静默功能失效**。
    **必须与排期方案 `架构改造排期方案_H4-H5_2026-09-13.md` 交叉核对（方案为准）+ 逐方法人工确认边界**；
    **⚠️ 方案本身也会错**（行号全漂、行数自相矛盾、把「已删的 `check_excel_table_needs`」列为待处置死代码、
    「与 `desktop_interaction` 重复」实为**分层**不是重复）→ **两边都验**。
  ...
  - ❗❗**W1 搬运铁律 8 条（全属"逐字等价测不出来"）** —— **全文见参考 §8.2/§8.3/§8.4**：
    ① `self` 当对象传出去救不了 → `QTimer(self.p)`；② 控制器**必须实现 `__setattr__`**（判据＝宿主**已拥有**；
  **必跳过 `'p'`**）；③ **新状态名在宿主 `init_systems()` 预声明**（**不是 `__init__`**）；④ `_log` 用
  `_log_()` 取宿主模块同一对象；⑤ **锁的定位器跟着代码搬家**；⑥ **`_log.` → `self._log_().`**；
  ⑦ **`os.`/`time.`/`random` 是"裸 Attribute 根"**，只扫"裸 Call 的 name"会**完全看不见**（W1-6 有
  **15 处**裸 `os.` 但方法体**自身无 `import os`**）→ 逐个确认有**模块级** import；⑧ **桩打在「方法体里
  `self` 的那张类」上**（见 §5 末）+ **夹具噪声查表归一**。
  - ★★ **A/B 的结构性盲区**：`types.MethodType(fn, stub)` 下 `self` 即 **stub** ⇒ **铁律 7/1/2/3 在 A/B 中
...
- **遗留**：① **真机实测（用户做）**：遮挡 / 上下动手感 / 边缘掉落不生气而关窗生气 —— **契约级+单元级已锁，
  ...
  **旧诊断实测不成立** → **不凭"应该会坏"改全局日志行为**；⑤ S7 第二批 ~11 处**不阻塞**；
  ⑥ **W1-6 的 B 块 3 个 `check_*_content` 暂缓**，待 H4「文件反应」专项；⑦ `scan_zero_refs.py` 的 `WAVE1`
  静态清单建议后续清理（W1-5 组含已删方法）。
